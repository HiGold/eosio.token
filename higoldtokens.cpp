...
// Standard EOS Token
ACTION higoldtokens::create( name   issuer,
...
ACTION higoldtokens::issue( name to, asset quantity, string memo )
{
...
ACTION higoldtokens::retire( name owner, asset quantity, string memo )
{
  auto sym = quantity.symbol;
  eosio_assert( sym.is_valid(), "invalid symbol name" );
  eosio_assert( memo.size() <= 256, "memo has more than 256 bytes" );
...
ACTION higoldtokens::transfer( name    from,
                               name    to,
                               asset   quantity,
                               string  memo )
{
...
ACTION higoldtokens::ret_balance( name owner, name ram_payer, asset value )
{
...
ACTION higoldtokens::sub_balance( name owner, asset value )
{
  accounts from_acnts( _self, owner.value );
...
ACTION higoldtokens::add_balance( name owner, asset value, name ram_payer )
{
...
ACTION higoldtokens::open( name owner, const symbol& symbol, name ram_payer )
{
...
// Proof of Stake
ACTION higoldtokens::startpos( asset  base_token,
                               asset  base_stake,
                               double weight,
                               string memo)
{
...
ACTION higoldtokens::pos(name owner, const symbol& symbol)
{
  auto sym = symbol;
  eosio_assert( sym.is_valid(), "invalid symbol name" );
  ...
  if( pos != pos_acnts.end() ) {
    ...
    double age    = (now() - p.timestamp) / (3600*24.0);
...
ACTION higoldtokens::buystake( name buyer, asset quantity )
{
  require_auth( buyer );
  ...
  string memo = "Buy Stake & Get: "+std::to_string((bought.amount)/10000.0)+" Stake";
  SEND_INLINE_ACTION( *this, transfer, { {buyer, "active"_n} },{ buyer, st.issuer, fee, memo });

}


ACTION higoldtokens::sellstake( name seller, asset stake )
{
  require_auth( seller );
  ...
  double age    = (now() - a.timestamp) / (3600*24);
  ...
  string memo = "Sell Stake & Get: "+std::to_string((get.amount-1)/10000.0)+" HIG";
  SEND_INLINE_ACTION( *this, transfer, { {seller, "active"_n} },{ seller, st.issuer, msgfee, memo });

}

ACTION higoldtokens::sub_stake( name owner, asset value )
{
...
ACTION higoldtokens::add_stake( name owner, asset value, name ram_payer )
{
